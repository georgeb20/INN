...
def inn_forward(inn_network, device, dataloader, epoch: int,
    ...
    in_train_ts = in_train_ts.to(device)
    out_train_ts = out_train_ts.to(device)
    in_test_ts = in_test_ts.to(device)
    out_test_ts = out_test_ts.to(device)

    val_window   = []
    PATIENCE     = 5
    DELTA_FRAC   = 0.002       # 0.2 % minimum improvement
    plateau_ctr  = 0
    grad_history = []
    best_loss = float('inf')
    ...
    for i in range(epoch):
        ...
        for step, (x_data, y_data) in enumerate(dataloader):
            ...
            loss.backward()
            grad_sum += torch.sqrt(sum(
                p.grad.norm()**2 for p in inn_network.parameters()
                if p.grad is not None
            )).item()
            n_steps += 1
            torch.nn.utils.clip_grad_norm_(inn_network.parameters(), max_norm=1.0)
            optimizer.step()
        grad_history.append(grad_sum / n_steps)   # mean over epoch
        ...
        if i % 1 ==0:
            with torch.no_grad():
                ...
                L_pred_t = 1 * nn.MSELoss()(y_pred_t[:,-y_dim:], out_test_ts[:,-y_dim:])
                L_inv_t = 1 * utl.mmd(x_inv_t[1:100,:x_dim],in_test_ts[1:100,:x_dim])
                L_latent_t = 1 * utl.mmd(y_pred_t[1:100,:z_dim],out_test_ts[1:100,:z_dim])
                L_pad_t = 1 * nn.L1Loss()(x_inv_t[:,-e_dim:],in_test_ts[:,-e_dim:])
                L_boundary_t = 1 * utl.boundary_loss(x_inv_t[:,:x_dim], in_test_ts[:,:x_dim])
            ...
            print("Epoch: " + str(i) + 
                ...
                " L_pred: " + str(L_pred.detach().cpu().numpy()) + 
                " L_inv: " + str(L_inv.detach().cpu().numpy()) + 
                " L_latent: " + str(L_latent.detach().cpu().numpy()) + 
                " L_pad: " + str(L_pad.detach().cpu().numpy())  +
                " L_Boundary: " + str(L_boundary.detach().cpu().numpy()) + 
                "\n"+
                " loss_t: " + str(loss_t.detach().cpu().numpy()) + 
                " L_pred_t: " + str(L_pred_t.detach().cpu().numpy()) + 
                " L_inv_t: " + str(L_inv_t.detach().cpu().numpy()) +
                " L_latent_t: " + str(L_latent_t.detach().cpu().numpy()) + 
                " L_pad_t: " + str(L_pad_t.detach().cpu().numpy()) +
                " L_Boundary_t: " + str(L_boundary_t.detach().cpu().numpy())
            )


        val_window.append(loss_t.item())
        if len(val_window) > 5:
            val_window.pop(0)
        if len(val_window) == 5:
            slope = (val_window[0] - val_window[-1]) / val_window[0]
            plateau_ctr = plateau_ctr + 1 if slope < DELTA_FRAC else 0
        ...
        if plateau_ctr >= PATIENCE:
            print(f"[Early-stop] validation plateau at epoch {i}")
    ...
    np.savez(save_folder + 'diagnostics_'+ts+'.npz',
            train=train_losses, test=test_losses, grad=np.array(grad_history))
    return train_losses, test_losses, grad_history
